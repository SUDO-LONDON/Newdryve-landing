...
export default function InstructorBilling({ applications }: { applications: InstructorApplication[] }) {
  const active = applications.filter((item) => ["active", "trialing"].includes(item.tenants?.subscription_status || "")).length;
  ...
  const contractedPence = applications
    .filter((item) => ["active", "trialing"].includes(item.tenants?.subscription_status || ""))
...
function BillingRow({ application }: { application: InstructorApplication }) {
  ...
  return (
    <tr className="align-top">
      ...
      <td className="px-4 py-4"><BillingPill status={tenant?.subscription_status || "none"} awaiting={application.status === "payment_pending"} /></td>
      <td className="px-4 py-4">
        {editing ? (
      ...
      <td className="px-4 py-4 text-ink-secondary">{date(tenant?.subscription_current_period_end)}</td>
...
function BillingPill({ status, awaiting }: { status: string; awaiting: boolean }) {
  const label = awaiting && status === "none" ? "Awaiting setup" : status.replaceAll("_", " ");
